...
for playlistID in playlistIDs:
    ...
    while playlistItems['items']:
        tracks = [t['track']['id'] for t in playlistItems['items']]
        res = sp.audio_features(tracks)
        print("tracks: ",len(tracks), ", res: ",len(res))
        for track in res:
            if track is None:
                print(playlistID, pn['name'])
                continue
            feature, trackId = track['energy'], track['id']

            entry = (feature, trackId)
            if sortAtEnd:
                topTen.append(entry)
            else:
                if len(topTen) < 10:
...
if sortAtEnd:
    if topK:
        while True:
            ...
            pivot = topTen[random.randint(0,len(topTen)-1)]
            # sort on each side
            left = []
            right = []
            for i in topTen:
                if i[0] >= pivot[0]:
                    ...
                    right.append(i)
            # if left side is shorter than 10, do it again
            if len(left) < 10:
                pass
            elif 10 <= len(left):
                topTen = left
    ...
    temp = sorted(topTen, reverse=True)
    for i in range(min(len(temp), 10)):
        trackName = sp.track(temp[i][1])['name']
        print(temp[i], trackName)
    
else:
    for i in range(len(topTen)):
        trackName = sp.track(topTen[i][1])['name']
        print(topTen[i], trackName)
        
end = time.time()
print(startTime, end, end - startTime)
